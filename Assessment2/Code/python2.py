...
def three(a):
	...
	three= str(a)*3
	four = str(a)*4
	return (int(a) + int(two) +int(three) + int(four))
...
def seven(a, b, c):
	list = [a, b, c]
	list.sort()
	diff_1 = list[1] - list[0]
	diff_2 = list[2] - list[1]
	print(diff_1)
	print(diff_2)
	if diff_1 == diff_2:
		return True
...
def eight(input,  a):
	...
	new_input = input[0:side] + input[end:]
	return new_input
...
def nine(string1, string2):
	...
	check_2 = all(item in list2 for item in list1)
	if check is True or check_2 is True:	
		return True
	else:
		return False
...
def ten(X,Y):
	...
	for numbers in range(X):
		for lists in range(Y):
			list2.append(numbers)
			list1.append(list2[lists])
	return list1
